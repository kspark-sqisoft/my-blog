...
| PATCH | `/api/auth/me` | 프로필(이름·아바타·소개) 수정 | 로그인 |
...
## 버그 수정 워크플로 (신규 기능과 다름)

간단한 버그는 10-Phase / 4-Phase 의 무거운 기획 절차를 건너뛴다.
**생략한다**: PRD/TRD·glossary·BC, 새 ADR, `feature_list.json` 태스크 분해(`/tasks`).
즉 버그 1건에 새 `T-...` ID 를 만들 필요는 없다.

> **단, 버그가 아키텍처 결함을 드러내면**(경계 위반·계약 불일치 등) 단순 수정으로 끝내지 말고 **큰 변경으로 승격**한다:
> 다음 번호 ADR 을 새로 쓰고, 기존 결정을 무효화하면 `supersede` 한다(절대규칙 #6 — ADR 은 immutable, 고치지 말고 새 번호.
> 예: ADR-0021 이 ADR-0003 을 supersede). 이때부터는 신규 기능 절차(태스크 분해·`/finish`)를 탄다.

버그라도 **절대 생략하지 않는 3가지**:

1. **진단 우선** — 바로 고치지 않는다. plan 모드 + `superpowers:systematic-debugging`(또는 `debugger` 서브에이전트)로
   **재현 → 근본원인**부터 확정한다. 추측 패치 금지(위 "CI/prod 실패 디버깅 — 추측 패치 금지" 원칙과 동일).
2. **회귀 테스트** — 버그를 **재현하는 실패 테스트를 먼저** 쓴다(RED 확인) → 수정 → GREEN. 통합/e2e 는 **`blog_test` DB 만**(절대규칙 #8).
   **회귀 테스트 없는 버그 수정은 금지** — TDD 가드(#7)·`review-gate` 정신과 동일. 파일·정적 리소스 버그면 쓰기-읽기 왕복까지(절대규칙 #9).
3. **커밋** — 버그도 커밋은 항상. 검증(lint/typecheck/test) 통과 + `code-reviewer` 경유 후 커밋한다
   (절대규칙 #3, Stop 훅 `review-gate` 가 `packages/*/src` 수정의 미리뷰 커밋을 차단).

**문서 환류 (심각도별)**:
- 일반 버그 → **회귀 테스트가 곧 문서**. 별도 문서 갱신 불필요.
- 반복되는 패턴(같은 함정 재발) → CLAUDE.md 의 주의/함정 섹션(예: "@blog/shared 순수성", "데이터 입력 주의")이나
  `docs/harness.md` 운영 함정에 **한 줄 추가**.
- 아키텍처 결함 → 위처럼 **ADR 로 승격**.

**범위 밖 발견 처리 (중요 — 범위 규율 절대규칙 #2)**: 버그를 고치다 **별개**의 문제/개선점을 발견하면 "지금" 고치지 말고 **기록**한다:
- 직결된 사소한 것(같은 함수 오타 등) → **같은 수정에 포함**.
- 별개의 새 버그 → `feature_list.json` 에 새 항목 추가(`status: "todo"`) + `docs/tasks/{feature}.md` 미러 동기화(절대규칙 #10).
- 개선 아이디어 → 같은 방식으로 `feature_list.json`(또는 백로그)에 기록.
- 세션 끝까지 못 끝낸 일 → `docs/handoff/{날짜}-*.md` 의 **"다음 세션이 알아야 할 것"** 에 명시.
→ **한 번에 하나**(범위 규율)를 지키면서 발견이 유실되지 않게 한다.

> **`/finish` 와의 관계**: 버그 수정이 `feature_list.json` 태스크 없이 진행되면 `/finish {ID}` 의 status=done 갱신 단계는 해당 없음이다.
> 검증 재실행 + `code-reviewer` + handoff 작성까지는 동일하게 거치되, 커밋 메시지는 `fix({scope}): ...` 로 남긴다.
> 버그를 태스크로 승격했다면(위 아키텍처 결함 경로) 그때는 신규 기능과 동일하게 `/finish {ID}` 로 마감한다.
