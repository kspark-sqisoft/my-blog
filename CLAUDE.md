...
> 하네스가 이 루프를 강제한다: TDD 가드 훅(UserPromptSubmit) → 자동 포맷/린트 훅(PostToolUse) → 검증-없는-done 차단 훅(Stop). 우회하지 말 것.
>
> 보조 도구(선택): `pnpm gc` — 미사용 코드(knip) + 중복(jscpd) 센서(비차단, CI `gc` job 과 동일). 리뷰/디버깅은 프로젝트 서브에이전트 `code-reviewer`·`debugger`(`.claude/agents/`)를 별도 패스로 호출.
