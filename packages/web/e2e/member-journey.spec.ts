...
test('회원이 실명으로 댓글을 남기고, AUTHOR 승격 후 글을 작성해 소유권을 가진다', async ({
  ...
  await admin.getByLabel('제목').fill(seedTitle);
  await admin.getByLabel('본문(마크다운)').fill('# 본문\n\n내용입니다.');
  ...
  await author.getByLabel('제목').fill(title);
  await author.getByLabel('본문(마크다운)').fill('# 소유권 본문\n\n내용입니다.');
