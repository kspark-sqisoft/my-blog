...
## 시리즈 (Series)- 정의: 한 작성자가 **순서 있게 묶은 Post 모음**. 연재물(예: "React 입문 1·2·3편")을 하나의 주제로 엮는다. 제목·설명(선택)·소유 작성자를 가지며, **한 Post 는 최대 한 시리즈에만** 속한다(series, ADR-0029). 발행된 글만 공개 시리즈에 노출(초안 비노출). URL 식별자는 슬러그(Post 와 동형, ADR-0022).- 소속 Context: Publishing (신규 Aggregate Root)- 코드 표현: `Series` (Prisma 모델), `Post.seriesId`(nullable FK)+`Post.seriesOrder`(시리즈 내 순서). `GET/POST /api/series`, `GET/PATCH/DELETE /api/series/:id` — ADR-0029- UI 표현: "시리즈" / 시리즈 상세(`/series/:slug`)·목록(`/series`)·글 상세의 시리즈 네비- 동의어 금지: 연재(통칭은 "시리즈"), 컬렉션(Collection), 모음집, 카테고리(Category — 분류는 Tag) — "시리즈"로 통일

## 시리즈 순서 (Series Order)- 정의: 한 시리즈 안에서 Post 들이 읽히는 **논리적 순서**. 작성자가 수동 지정하며(발행일과 무관), 시리즈 상세 목록·글 상세 네비의 정렬 기준이다(ADR-0029).- 소속 Context: Publishing- 코드 표현: `Post.seriesOrder: number`(시리즈 미소속이면 무의미), 시리즈 내 정렬 키- UI 표현: 시리즈 관리 화면의 순서 조정, 상세의 "N / M편"- 동의어 금지: 인덱스(Index), 번호(범용) — "시리즈 순서"로 통일

## 시리즈 네비게이션 (Series Navigation)- 정의: 글 상세에서 **이 글이 속한 시리즈와 그 안의 위치(N/M편)·이전/다음 글**을 보여주는 이어보기 동선(ADR-0029). 시리즈 내 발행글만 대상으로 한다.- 소속 Context: Publishing (발행글 프레젠테이션 읽기 모델 — Post 상세에서 파생)- 코드 표현: `PostDetailDto` 의 시리즈 메타(시리즈 제목·슬러그·현재 위치·이전/다음 글 요약) — 형태는 TRD- UI 표현: 글 상세 본문 위/아래의 "X 시리즈 · N/M편" + 이전/다음 글 링크- 동의어 금지: 페이지네이션(목록 페이징과 구분) — "시리즈 네비게이션"으로 통일
